...
class SearchOutCgController extends Controller
{
    public $refererNumPlace;
    const refererNum = 3;

    function __construct(){
        $this->refererNumPlace = SearchOutCgController::refererNum;
    }//__construct

    //
    public function index(Request $request){
        $call = new SearchOutCgController;
        $search = @$_GET['search'];
        $items = $call->show($this->refererNumPlace);
        // return $items; teste
        $outCg = true;
        return view('legaladvice.registries.index', compact('items', 'search', 'outCg'));
    }//index()
    ...
    private function show($id){
        ...
        $outOfCg = 
            ...
            where r.place=$id /* busca somente fora do cg. cg = 3 */
