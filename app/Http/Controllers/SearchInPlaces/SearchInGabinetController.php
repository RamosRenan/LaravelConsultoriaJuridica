...
class SearchInGabinetController extends Controller
{
    //code ...

    public $refererNumPlace;
    const refererNumGabinet = 2;

    function __construct(){
        $this->refererNumPlace = SearchInGabinetController::refererNumGabinet;
    }//__construct

    //
    public function index(Request $request){
        // return $this->refererNumPlace;
        $call = new SearchInGabinetController;
        $search = @$_GET['search'];
        $items = $call->show($this->refererNumPlace);
        $gabinet = true;
        return view('legaladvice.registries.index', compact('items', 'search', 'gabinet'));
    }//index()
    ...
    private function show($id){
        ...
        $outOfCg = 
            ...
            where r.place=$id /* busca somente fora do cg = 2 */
