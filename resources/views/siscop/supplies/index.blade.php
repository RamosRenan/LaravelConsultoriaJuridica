...
@section('content_header')
    <h1><i class="fa fa-list"></i> @lang('global.dentist.supplies_items.title') <small>@lang('global.app_list')</small></h1>
@stop

@section('js') 
    <script>
        var route = '{{ route('dentist.supplies.mass_destroy') }}';
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <div class="row">
                <div class="col-md-8">
                    <p>
                        <a href="{{ route('dentist.supplies.create') }}" class="btn btn-success"><i class="fa fa-plus"></i> @lang('global.app_create')</a>
                    </p>
                </div>
                <div class="col-md-4">
                    {!! Form::open(['style' => 'display: inline-block;', 'method' => 'GET', 'route' => ['dentist.supplies.index']]) !!}
                    ...
                    @if (count($items) > 0)
                        @foreach ($items as $item)
                            <tr data-entry-id="{{ $item->id }}">
                                ...
                                <td>{{ $item->name }}</td>
                                <td>
                                    <a href="{{ route('dentist.supplies.edit',[$item->id]) }}" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i> @lang('global.app_edit')</a>
                                    {!! Form::open(array(
                                        'style' => 'display: inline-block;',
                                        'method' => 'DELETE',
                                        'onsubmit' => "return confirm('".trans("global.app_are_you_sure")."');",
                                        'route' => ['dentist.supplies.destroy', $item->id])) !!}
