...
@section('content_header')
    <h1><i class="fa fa-ambulance"></i> @lang('global.dentist.units.title') <small>@lang('global.app_list')</small></h1>
@stop

@section('js') 
    <script>
        var route = '{{ route('dentist.units.mass_destroy') }}';
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <div class="row">
                <div class="col-md-8">
                    <p>
                        <a href="{{ route('dentist.units.create') }}" class="btn btn-success"><i class="fa fa-plus"></i> @lang('global.app_create')</a>
                    </p>
                </div>
                <div class="col-md-4">
                    {!! Form::open(['style' => 'display: inline-block;', 'method' => 'GET', 'route' => ['dentist.units.index']]) !!}
            ...
            <table class="table table-bordered table-striped dt-select">
                <thead>
                    <tr>
                        ...
                        <th>@lang('global.dentist.units.fields.name')</th>
                        <th>@lang('global.dentist.units.fields.code')</th>
                        <th>&nbsp;</th>
                    </tr>
                </thead>
                    ...
                    @if (count($items) > 0)
                        @foreach ($items as $item)
                            <tr data-entry-id="{{ $item->id }}">
                                ...
                                <td>{{ $item->code }}</td>
                                <td>
                                    <a href="{{ route('dentist.units.edit',[$item->id]) }}" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i> @lang('global.app_edit')</a>
                                    {!! Form::open(array(
                                        'style' => 'display: inline-block;',
                                        'method' => 'DELETE',
                                        'onsubmit' => "return confirm('".trans("global.app_are_you_sure")."');",
                                        'route' => ['dentist.units.destroy', $item->id])) !!}
