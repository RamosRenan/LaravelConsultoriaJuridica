...
@section('content_header')
    <h1><i class="fa fa-info"></i> @lang('legaladvice.statuses.title')</h1>
@stop
...
<script>
    ...
    $( "table tbody" ).sortable({
        ...
        update: function( event, ui ) {
            $.ajax({
                url: '{{ route("legaladvice.statuses.order") }}',
                type: 'get',
    ...
    var route = '{{ route('legaladvice.statuses.mass_destroy') }}';
</script>
@stop
...
@section('content')
    <div class="card card-default">
        <div class="card-header">
            <div class="row">
                <div class="col-md-6">
                    <a href="{{ route('legaladvice.statuses.create') }}" class="btn btn-success btn-sm"><i class="fa fa-plus"></i> @lang('global.app_create')</a>
                </div>
                <div class="col-md-6">
                    <div class="float-sm-right">
                        {{ Form::open(['method' => 'GET', 'route' => ['legaladvice.statuses.index']]) }}
            ...
            <table class="table table-head-fixed table-hover">
                <thead>
                    <tr>
                        ...
                        <th>@lang('legaladvice.statuses.fields.name')</th>
                        <th>&nbsp;</th>
                    </tr>
                </thead>
                <tbody>
                ...
                <tr id="{{ $item->id }}" data-entry-id="{{ $item->id }}" class="item-order">
                    ...
                    <td class="align-middle text-right">
                        {{ Form::open(array(
                            ...
                            'method' => 'DELETE',
                            'route' => ['legaladvice.statuses.destroy', $item->id])) }}
                        {{ Form::close() }}
                        <a href="#" class="btn btn-sm btn-secondary order-handle"><i class="fa fa-arrows-alt"></i></a>
                        <div class="btn-group">
                            <a href="{{ route('legaladvice.statuses.edit',[$item->id]) }}" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i> @lang('global.app_edit')</a>
